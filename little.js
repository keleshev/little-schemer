...
(function() {
  var Cell, DELIMITER, List, cond, define, eval_operands, lookup, provide, set,
    __slice = [].slice;
  ...
  define = function(expr, env) {
    ...
    return frame.cdr.car = Cell(dval, frame.cdr.car);
  };
  ...
  Cell = (function() {
    ...
    Cell.prototype["eval"] = function(env) {
      ...
      if (expr.self_evaluating != null) {
        return expr;
      } else if ((expr.pair != null) && expr.car.symbol === 'quote') {
        return expr.cdr.car;
      } else if (expr.symbol != null) {
        return lookup(expr, env);
      } else if ((expr.pair != null) && expr.car.symbol === 'define') {
        define(expr, env);
        return Cell('ok');
      } else if ((expr.pair != null) && expr.car.symbol === 'set!') {
        set(expr, env);
        return Cell('ok');
      } else if ((expr.pair != null) && expr.car.symbol === 'cond') {
        return cond(expr.cdr, env);
      } else if ((expr.pair != null) && expr.car.symbol === 'env') {
        return env;
      } else if ((expr.pair != null) && expr.car.symbol === 'lambda') {
        return Cell({
          procedure: expr,
          env: env
        });
        ...
        if (operator.primitive != null) {
          return Cell(operator.primitive(args));
        } else if (operator.procedure != null) {
    ...
    Cell.evaluate__ = function(source) {
      var env, evaled, parsed, result, _ref;
      env = Cell.default_env();
      result = '';
      while (source !== '') {
        _ref = Cell._read(source), parsed = _ref[0], source = _ref[1];
    ...
    Cell.default_env = function() {
      var env, func, name, _ref;
      env = this.read('((() ()))');
      _ref = this._primitives;
      for (name in _ref) {
        func = _ref[name];
    ...
    Cell.prototype.write = function() {
      if (this.pair != null) {
        ...
        return "<function " + (this.procedure.write()) + ">";
      } else {
        throw 'write error';
      }
    };
